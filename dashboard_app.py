...
def load_data():
    """
    Loads the pre-processed data from the 'ava_queries_processed.csv' file.
    This file is generated by the separate 'preprocess_data.py' script.
    """
    processed_file_path = "results/gemini_classification/ava_queries_processed.csv"
    st.sidebar.info(f"📊 Loading pre-processed data...")
    try:
        df = pd.read_csv(processed_file_path)
    except FileNotFoundError:
        st.error(
            f"**Processed data file not found at `{processed_file_path}`.**\n\n"
            "Please run the `preprocess_data.py` script first to generate the required file."
        )
        st.stop()
    
    # The pre-processing script handles most cleaning, but we still need to
    # ensure data types are correct for plotting, especially dates.
...
try:
    ...
    st.error(
        "**Failed to connect to Snowflake.** This is likely an authentication error. "
        "Please double-check your credentials in the Streamlit secrets. "
        f"Snowflake error: {e}"
    )
    st.stop()
